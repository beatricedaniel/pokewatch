...
def calculate_metrics(
    ...
    for _, row in df.iterrows():
        ...
        true_fair_price = row["fair_value_baseline"]
...
def create_visualizations(pred_df: pd.DataFrame, output_dir: Path) -> dict:
    ...
    min_price = min(
        pred_df["true_market_price"].min(), pred_df["predicted_fair_price"].min()
    )
    max_price = max(
        pred_df["true_market_price"].max(), pred_df["predicted_fair_price"].max()
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train/evaluate baseline model with MLflow tracking")
    parser.add_argument(
        "--data_path",
        type=str,
        default=None,
